...
Fixes (2026-07-26):
  ...
  shorter than 30 seconds, ffmpeg seeks past EOF → returns None → the caller's combined
  `or` check printed "ffmpeg not available" and returned True (PASS). Every sub-30s
  reel silently bypassed protagonist identity checking.
  Fix: probe duration via ffprobe, compute closing_ts = max(1s, min(duration-1s, 30s)),
  split opening/closing None checks into distinct diagnostic branches, add
  probed_duration_seconds to the QA JSON report.
...
def get_video_duration(video_path: Path) -> Optional[float]:
    ...
    Returns float seconds, or None if ffprobe is unavailable / fails.

    FIX Bug G: Used to compute a safe closing timestamp for short reels
    so we never seek past EOF.
    """
    try:
...
def _seconds_to_timestamp(seconds: float) -> str:
    """Convert float seconds to HH:MM:SS string for ffmpeg -ss."""
    h = int(seconds // 3600)
...
def verify_protagonist(
    ...
    FIX Bug G (CRITICAL): Closing frame timestamp was hardcoded to 00:00:30. For
    any reel shorter than 30 seconds, ffmpeg seeks past EOF → returns None → the
    caller's `or` check printed "ffmpeg not available" and returned True (PASS).
    Fix: probe duration with ffprobe; compute closing_ts = max(1s, min(duration-1s, 30s));
    split opening/closing None checks into distinct diagnostic branches.
    """
    ...
    try:
        ...
        # FIX Bug G: Probe actual duration to compute a safe closing timestamp.
        ...
        if probed_duration is not None:
            ...
            closing_ts = _seconds_to_timestamp(closing_secs)
            print(
                f"  ⏱  Probed duration: {probed_duration:.1f}s "
                f"→ closing timestamp: {closing_ts}"
            )
        else:
            closing_ts = "00:00:30"
            print("  ⚠️   Could not probe duration via ffprobe; using fallback t=00:00:30")
    ...
    # FIX Bug G: Separate diagnostic branches so we know WHICH frame failed,
    # rather than collapsing both into a misleading "ffmpeg not available" message.
    if opening_frame is None:
        print("  ⚠️   Could not extract opening frame (ffmpeg unavailable or file corrupt)")
    ...
    if closing_frame is None:
        print("  ⚠️   Could not extract closing frame (ffmpeg unavailable or file corrupt)")
        print("  ℹ️   Install ffmpeg to enable face-identity checking")
